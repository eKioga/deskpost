...
**173 rows** across 18 areas: 154 compared against PowerShell, 19 judged independently. 30 need a reachable shared collection and are skipped offline. 23 public helpers are excluded with a reason rather than given a row.
...
| --- | --- | --- | --- | --- |
| `workspace.init-creates-marker` | success | differential | `bare-folder` | `library init` on an empty folder writes the marker, registers the workspace, and installs the managed instruction and harness files. _(delta: kernel-replaces-the-library-adapter-entry; delta: kernel-serves-a-local-workspaces-reader)_ |
| `workspace.init-is-idempotent` | success | differential | `workspace-fresh` | A second `library init` over the same folder keeps the workspace id and its created stamp, and reports that rather than reissuing them. _(delta: kernel-replaces-the-library-adapter-entry; delta: kernel-serves-a-local-workspaces-reader)_ |
