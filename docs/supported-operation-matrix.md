...
| --- | --- | --- | --- | --- |
| `workspace.init-creates-marker` | success | differential | `bare-folder` | `library init` on an empty folder writes the marker, registers the workspace, and installs the managed instruction and harness files. _(delta: kernel-registers-its-own-hooks; delta: kernel-replaces-the-library-adapter-entry; delta: kernel-serves-a-local-workspaces-reader)_ |
| `workspace.init-is-idempotent` | success | differential | `workspace-fresh` | A second `library init` over the same folder keeps the workspace id and its created stamp, and reports that rather than reissuing them. _(delta: kernel-registers-its-own-hooks; delta: kernel-replaces-the-library-adapter-entry; delta: kernel-serves-a-local-workspaces-reader)_ |
...
| `workspace.init-force-refreshes-managed-sections` | success | differential | `workspace-fresh` | `library init -Force` refreshes the managed instruction sections and the marker's program version against an existing workspace, leaving the reader's own prose alone. _(delta: kernel-registers-its-own-hooks; delta: kernel-replaces-the-library-adapter-entry; delta: kernel-serves-a-local-workspaces-reader)_ |
| `workspace.init-leaves-a-workspace-its-checks-pass` | success | differential | `bare-folder` | `library init` on an empty folder lays out the Holding Shelf and the Report Inbox as capture Books, renders the Shelf catalog from them and an empty Notebook master index, and the workspace checks that follow fail nothing (S42, the reader's ruling). _(delta: kernel-registers-its-own-hooks; delta: kernel-replaces-the-library-adapter-entry; delta: kernel-serves-a-local-workspaces-reader)_ |
...
| `kernel-serves-a-local-workspaces-reader` | `workspace.init-creates-marker`, `workspace.init-is-idempotent`, `workspace.init-force-refreshes-managed-sections`, `workspace.init-leaves-a-workspace-its-checks-pass` | `^effect\.\.(mcp\.json\|codex/config\.toml)$` | S46, ADR-0044: a compiled kernel attached to its local collection registers itself -- bin/library mcp serve -- as the workspace's reader for Claude and Codex, where the PowerShell initializer registers its adapter, which reads Basic Memory only and so refused a Tier 0 seat's first Project read in Windows Sandbox. Only the two reader registrations differ; every other file init writes is compared. Judged by kernel self-test section 36, which launches the registered reader and reads a Project page through it. | the reader, S46, choosing to fix the Windows workspace reader with the init rows' delta, 2026-09-24 |
| `kernel-replaces-the-library-adapter-entry` | `workspace.init-creates-marker`, `workspace.init-is-idempotent`, `workspace.init-force-refreshes-managed-sections`, `workspace.init-leaves-a-workspace-its-checks-pass` | `^result\.files\[(2\|6)\]\.action$` | S46: re-run over a workspace whose reader is the Library's own adapter entry, the kernel replaces it with itself and reports that file merged where the PowerShell initializer reports it unchanged. files[2] is .mcp.json and files[6] .codex/config.toml in init's fixed order; only those two actions, and only between unchanged and merge. | the reader, S46, choosing to fix the Windows workspace reader with the init rows' delta, 2026-09-24 |
| `kernel-registers-its-own-hooks` | `workspace.init-creates-marker`, `workspace.init-is-idempotent`, `workspace.init-force-refreshes-managed-sections`, `workspace.init-leaves-a-workspace-its-checks-pass` | `^(effect\.\.(claude/settings\.local\.json\|codex/hooks\.json)\|result\.files\[(4\|5)\]\.action)$` | S48, ADR-0046: on Windows a compiled kernel's init registers its own five ported hooks -- bin/library.exe hook <verb> in exec form for Claude, & "<program>/bin/library" hook <verb> for Codex, which runs a hook through powershell.exe -- where the PowerShell initializer registers the guard scripts, so a closed-Book denial on the default route named tools/Set-VirtualDesk.ps1 and ADR-0045's rewrite never ran (S47, S7 in Windows Sandbox). The four hooks with no kernel port stay PowerShell. Only the two hook files differ, and their two actions (files[4] is .claude/settings.local.json and files[5] .codex/hooks.json in init's fixed order, only between unchanged and merge); .codex/hooks.json must still register all four required guards on each side, by script or by verb. .claude/settings.local.json is over the inline limit and compared by hash, so this delta cannot see inside it: section 39 is what judges its content. Every other file init writes is compared. Judged by kernel self-test section 39, which launches the registered hooks against a closed Book. | the reader, S48, ruling that a compiled Windows init registers the kernel's own hooks and approving this delta by id, 2026-09-25 |
